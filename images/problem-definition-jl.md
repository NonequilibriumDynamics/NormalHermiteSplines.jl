...
The Bessel potential space ``H^s_\varepsilon (R^n), \ (\varepsilon \gt 0 , \ s \gt \frac{n}{2} + r)`` is a Hilbert space, a function ``f`` from that space can be treated as a ``r``-times continuously differentiable function, i.e. as an element of ``C^r (R^n)``.
